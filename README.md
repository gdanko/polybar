...
* `disk-usage.py` - This script shows disk usage in the format: `<used> GiB / <total> GiB`. Future versions will allow a `--format` flag that will allow multiple formatting options. This script uses `df -B 1` to gather the data. It has an optional `--unit` flag that allows you to force the unit you want to display it in.
* `memory-usage.py` - This script shows memory usage in the format: `<used> GiB / <total> GiB`. Future versions will allow a `--format` flag that will allow multiple formatting options. This script uses `free -b -w` to gather the data. It has an optional `--unit` flag that allows you to force the unit you want to display it in.
...
* Add a `--format` flag to make it a little more fun
